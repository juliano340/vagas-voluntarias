...
export class VagasService {
  ...
  async create(dto: CreateVagasDto, user: User) {
    if (user.role !== 'ofertante') {
      throw new ForbiddenException(
        'Apenas usuários ofertantes podem criar vagas.',
      );
    }
  ...
  async findAll(filtros: { localidade?: string; titulo?: string }) {
    ...
    if (localidade) {
      query.andWhere('vaga.localidade ILIKE :localidade', {
  ...
  async remove(id: number, user: User) {
    ...
    if (vaga.publicadaPor.id !== user.id) {
      throw new ForbiddenException(
        'Você não tem permissão para remover esta vaga',
      );
    }
  ...
  async listarCandidatosDaVaga(vagaId: number, user: User) {
    ...
    if (vaga.publicadaPor.id !== user.id) {
      throw new ForbiddenException(
        'Você não tem permissão para ver os candidatos desta vaga.',
      );
    }
